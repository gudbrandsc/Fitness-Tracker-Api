...
module.exports = {
  ...
  listexerciseforuser(req, res) {
    ...
    .then(function(exercise_detailsNew) {
		...
		{
			promisearray.push(module.exports.getsessiondata(exercise_detailsNew[i].SessionId));
		}
   ...
   getmaxsessionvalue()
   {
	   ...
	   {
		    exercise_details.findAll({ 
			attributes: [ [Sequelize.fn('MAX', Sequelize.col('SessionId')) ,'SessionId']],
			})
	   ...
	   for(var j in details)
		{
			var jsonTemp = {
					...
					jsonTemp.categoryname = details[j].Workout_table.Workout_Category.CategoryName;
				
					jsonTemp.categoryurl = details[j].Workout_table.Workout_Category.ImageUrl;
